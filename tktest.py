...
check_1 = ttk.Checkbutton(check_frame, text="Unchecked", variable=a, 
        command=lambda: print(a.get()), onvalue="agree", offvalue="disagree")
check_1.grid(row=0, column=0, padx=5, pady=10, sticky="nsew")
check_2 = ttk.Checkbutton(check_frame, text="Unchecked", variable=b, 
        command=lambda: print(b.get()))
...
radio_1 = ttk.Radiobutton(radio_frame, text="Deselected", command=radio_select, 
                    variable=d, value=1)
radio_1.grid(row=0, column=0, padx=5, pady=10, sticky="nsew")
radio_2 = ttk.Radiobutton(radio_frame, text="Selected", command=radio_select,
                    variable=d, value=2)
...
entry = ttk.Entry(widgets_frame, textvariable=text)
entry.insert(0, "Entry")
entry.grid(row=0, column=0, padx=5, pady=(0, 10), sticky="ew")

# Button
button = ttk.Button(widgets_frame, text="Button", command=get_entry)
button.grid(row=1, column=0, padx=5, pady=10, sticky="nsew")

# Spinbox
def spin_value_change():
    print(f"Valor corrente: {current_value.get()} %")

current_value = tk.StringVar()

spinbox = ttk.Spinbox(widgets_frame, from_=0, to=100,
            textvariable=current_value, command=spin_value_change)
spinbox.insert(0, "SpinBox")
spinbox.grid(row=2, column=0, padx=5, pady=10, sticky="ew")

# Combobox

combobox = ttk.Combobox(widgets_frame, values=combo_list)
combobox.current(0)
combobox.grid(row=3, column=0, padx=5, pady=10,  sticky="ew")
...
readonly_combo.grid(row=4, column=0, padx=5, pady=10,  sticky="ew")

# bind the selected value changes
def  combo_changed(event):
    """ handle the combo changed event """
    showinfo(
        title='Result',
        message=f'You selected {readonly_combo.get()}!'
    )

readonly_combo.bind('<<ComboboxSelected>>', combo_changed)

# Menu for the Menubutton
menu = tk.Menu(widgets_frame)
menu.add_command(label="Menu item 1", command=lambda: print("command 1"))
menu.add_command(label="Menu item 2", command=lambda: print("command 2"))
menu.add_separator()
menu.add_command(label="Menu item 3", command=lambda: print("command 3"))
menu.add_command(label="Menu item 4", command=lambda: print("command 4"))

# Menubutton
menubutton = ttk.Menubutton(widgets_frame, text="Menubutton", menu=menu, 
            direction="below")
menubutton.grid(row=5, column=0, padx=5, pady=10, sticky="nsew")
...
optionmenu.grid(row=6, column=0, padx=5, pady=10, sticky="nsew")

# Accentbutton
accentbutton = ttk.Button(widgets_frame, text="Accentbutton",
             style="Accent.TButton", command=lambda: print(("Accent Button")))
accentbutton.grid(row=7, column=0, padx=5, pady=10, sticky="nsew")

# Togglebutton
def toggle():
    print(f"toggle status {tg.get()}")
    label["text"] = f"toggle status {tg.get()}"

tg = tk.BooleanVar()
button = ttk.Checkbutton(widgets_frame, text="Togglebtn", style="ToggleButton",
                variable=tg, command=toggle)
button.grid(row=8, column=0, padx=5, pady=10, sticky="nsew")


# Switch
def switch_changed():
    print(f"Switch changed to {sw.get()}")
sw = tk.BooleanVar()
switch = ttk.Checkbutton(widgets_frame, text="Switch", style="Switch",
             variable=sw, command=switch_changed)
